...
export default function AdminAppointments() {
  const { data: appointments, loading, refetch } = useSupabaseTable('appointments', {
    // --- THIS IS THE MAGIC LINE: It tells the database to fetch the client_name ---
    select: `
      id,
      appointment_date,
      start_time,
                ...
                <table className="data-table" style={{ margin: 0 }}>
                  <thead>
                    <tr>
                      ...
                      {!showHistory && <th style={{ width: '20%', textAlign: 'right' }}>Actions</th>}
                    </tr>
                  </thead>
                  <tbody>
                    {aptsForDate.map(apt => (
                      <tr key={apt.id}>
                        ...
                        {/* --- THIS IS WHERE THE NAME DISPLAYS --- */}
                        <td>
                        ...
                        <td style={{ fontWeight: '500' }}>{apt.services?.name}</td>
