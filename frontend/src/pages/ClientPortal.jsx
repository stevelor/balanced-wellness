...
export default function ClientPortal() {
  ...
  const [availability, setAvailability] = useState([]) // NEW: Store business hours
  ...
  useEffect(() => {
    fetchServices()
    fetchMyAppointments()
    fetchAvailability() // NEW: Grab the hours on load
  }, [])
  ...
  const handleBooking = async (e) => {
    ...
    // --- NEW BUSINESS LOGIC: VALIDATE HOURS ---
    
    // 1. Convert the chosen date string (YYYY-MM-DD) into a Javascript Day of Week (0-6)
    // We add T00:00:00 to prevent timezone shifting bugs
    const dayOfWeek = new Date(`${date}T00:00:00`).getDay() 

    // 2. Check if there are any working hours set for this specific day
    ...
    if (dayRules.length === 0) {
      ...
      return // Stop the function here, don't save to database
    }

    // 3. Check if the chosen time falls strictly between the start and end times
    // We can do direct string comparison because standard 24-hour time formats perfectly (e.g., "09:00" < "14:00")
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '40px auto', padding: '20px' }}>
      ...
      <div style={{ backgroundColor: '#e8f4f8', padding: '20px', borderRadius: '8px', marginBottom: '30px' }}>
        ...
        {/* Quick UI helper showing the business hours */}
